...
if exists('$TMUX') || has('gui')
    ...
    if strftime("%H") > 10 && strftime("%H") <= 16
        " 早上11点到下午5点为gruvbox dark背景
        set background=dark
        colorscheme gruvbox
    elseif strftime("%H") > 16 && strftime("%H") <= 22
        " 下午5点到晚上11点为molokai light背景
        set background=light
        colorscheme molokai
    elseif strftime("%H") > 22 || strftime("%H") <= 4
        " 晚上11点到凌晨5点为solarized dark背景
        set background=dark
        colorscheme solarized
    else
        " 凌晨5点到上午11点为molokai dark背景
        set background=dark
        colorscheme molokai
    endif
else
